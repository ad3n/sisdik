...
class SpreadsheetReader_XLSX implements \Iterator, \Countable
{
    ...
    private $Options = array(
        'TempDir' => '', 'ReturnDateTimeObjects' => false
    );
    ...
    private static $BuiltinFormats = array(
            ...
            12 => '# ?/?', 13 => '# ??/??', 14 => 'mm-dd-yy', 15 => 'd-mmm-yy', 16 => 'd-mmm',
            ...
            57 => '[$-404]e/m/d',
            // THA
            59 => 't0', 60 => 't0.00', 61 => 't#,##0', 62 => 't#,##0.00', 67 => 't0%', 68 => 't0.00%',
            69 => 't# ?/?', 70 => 't# ??/??'
    );
    ...
    private static $DateReplacements = array(
            'All' => array(
                    '\\' => '', 'am/pm' => 'A', 'yyyy' => 'Y', 'yy' => 'y', 'mmmmm' => 'M', 'mmmm' => 'F',
                    'mmm' => 'M', ':mm' => ':i', 'mm' => 'm', 'm' => 'n', 'dddd' => 'l', 'ddd' => 'D',
                    'dd' => 'd', 'd' => 'j', 'ss' => 's', '.s' => ''
            ), '24H' => array(
                'hh' => 'H', 'h' => 'G'
            ), '12H' => array(
    ...
    public function __construct($Filepath, array $Options = null) {
        ...
        $this->TempDir = isset($Options['TempDir']) && is_writable($Options['TempDir']) ? $Options['TempDir']
                : sys_get_temp_dir();
        ...
        if ($Status !== true) {
            throw new \Exception(
                    'SpreadsheetReader_XLSX: File not readable (' . $Filepath . ') (Error ' . $Status . ')');
        }
        ...
        if ($Zip->locateName('xl/styles.xml') !== false) {
            ...
            if ($this->StylesXML->numFmts && $this->StylesXML->numFmts->numFmt) {
                foreach ($this->StylesXML->numFmts->numFmt as $Index => $NumFmt) {
                    $this->Formats[(int) $NumFmt->attributes()->numFmtId] = (string) $NumFmt->attributes()
                            ->formatCode;
                }
            }
    ...
    private function PrepareSharedStringCache() {
        ...
        if (!$this->SharedStringCount
                || (self::SHARED_STRING_CACHE_LIMIT < $this->SharedStringCount
    ...
    private function GetSharedString($Index) {
        if ((self::SHARED_STRING_CACHE_LIMIT === null || self::SHARED_STRING_CACHE_LIMIT > 0)
                && !empty($this->SharedStringCache)) {
    ...
    private function FormatValue($Value, $Index) {
        ...
        if (!$Format) {
            ...
            if (substr($Format['Code'], -1) == '%') {
                ...
                $Format['Code'] = trim(
                        preg_replace('{^(\[\$[[:alpha:]]*-[0-9A-F]*\])}i', '', $Format['Code']));
                ...
                if (preg_match('{#?.*\?\/\?}', $Format['Code'])) {
                    ...
                    $Matches = array();
                    if (preg_match('{(0+)(\.?)(0*)}', preg_replace('{\[[^\]]+\]}', '', $Format['Code']),
                            $Matches)) {
                        $Integer = $Matches[1];
 ...
 elseif ($Format['Type'] == 'DateTime') {
                ...
                if ($Format['Type'] == 'Fraction' && ($Value != (int) $Value)) {
                    ...
                    if ($Format['MinWidth'] && $Format['Decimals']) {
                        if ($Format['Thousands']) {
                            $Value = number_format($Value, $Format['Precision'], self::$DecimalSeparator,
